...
@Composable
fun EnterEmailScreen(navController: NavController, onNavigateUp: () -> Unit) {
    Scaffold(
        topBar = {
        ...
        Column(
            ...
            Text(
                text = "Please enter your account's email",
                modifier = Modifier.padding(top = topPadding)
            )
            Button(onClick = {
                navController.navigate(Route.ForgetPasswordScreenOTPScreen.route)
            }) {
                Text(text = "Tiếp tục")
            }
        }
